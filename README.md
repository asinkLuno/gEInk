...
将图片转换为彩色点彩风格艺术图。四阶段流水线：均值漂移色块概括 → 7色 Atkinson 抖动 → 导出点坐标/颜色数据（原图分辨率） → Node.js Canvas multiply 混合渲染。
...
| `--dot-ratio` / `-r` | `0.01` | 点半径占图片短边的比例（0.01 = 1%） |
| `--jitter` / `-j` | `2` | 点坐标最大随机偏移像素数（打破机械排布） |
| `--alpha` / `-a` | `0.5` | 每个点的不透明度（0=全透明，1=不透明），multiply 混合下影响颜色深浅 |
...
首次使用需要为 `arduino-cli` 添加 ESP8266 第三方源并安装平台：

```bash
# 1. 添加 ESP8266 Board Manager URL
arduino-cli config add board_manager.additional_urls https://arduino.esp8266.com/stable/package_esp8266com_index.json

# 2. 更新索引
arduino-cli core update-index

# 3. 安装 ESP8266 平台（约 110 MiB，需要一些时间）
...
# 应显示：esp8266:esp8266  3.1.2  esp8266
```
...
| 资源 | 使用量 | 总量 | 占比 |
|------|--------|------|------|
| RAM (全局/静态) | 31,428 B | 80,192 B | 39% |
| IRAM (指令内存) | 60,523 B | 65,536 B | 92% |
| Flash (代码) | 301,352 B | 1,048,576 B | 28% |
...
- `/home/guozr/.cache/arduino/sketches/.../SimpleLoader.ino.bin` - 烧录用固件
...
| 资源 | 使用量 | 总量 | 占比 |
|------|--------|------|------|
| RAM (全局/静态) | 30,004 B | 80,192 B | 37% |
| IRAM (指令内存) | 60,807 B | 65,536 B | 92% |
| Flash (代码) | 292,744 B | 1,048,576 B | 27% |
